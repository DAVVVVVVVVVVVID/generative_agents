...
def run_gpt_prompt_task_decomp(persona,
  ...
  Takes a high-level task and breaks it down into specific subtasks,
  each with a duration in minutes.

  INPUT:
    persona: Persona instance
    task: String description of the task to decompose
    duration: Total duration in minutes for the task
    test_input: Optional test input to bypass persona data
  OUTPUT:
    List of [subtask_description, duration_in_minutes] pairs
  """
  ...
  def __chat_func_clean_up(gpt_response, prompt=""):
    ...
    # Make sure durations match the expected total
  ...
  def __chat_func_validate(gpt_response, prompt=""):
    try:
      result = __chat_func_clean_up(gpt_response, prompt)
      # Ensure we got a valid list of subtasks
  ...
  # Adjust last task duration to match total
  if len(fin_output) > 0:
    ftime_sum = 0
  ...
  # Combine main task with subtasks
  task_decomp = output
  ret = []
  for decomp_task, duration_min in task_decomp:
    ret += [[f"{task} ({decomp_task})", duration_min]]
  output = ret
...
def run_gpt_prompt_action_arena(action_description, persona, maze, act_world, act_sector,
  ...
  INPUT:
    ...
    persona: Persona instance
    maze: Maze/world instance
    act_world: World name
    act_sector: Sector name
  OUTPUT:
    Arena name where the action should take place
  """
  ...
  special_instruction = "The value for the output must contain one of the arena options above verbatim (including lower/upper case)."
...
def run_gpt_prompt_action_game_object(action_description, persona, maze, temp_address,
  ...
  special_instruction = "The value for the output must contain one of the object options above verbatim (including lower/upper case)."
...
def run_gpt_prompt_pronunciatio(action_description, persona, test_input=None, verbose=False):
  ...
  INPUT:
    ...
    persona: Persona instance
  OUTPUT:
    Emoji string (up to 3 characters)
  """
  def create_prompt_input(action_description, test_input=None):
    if test_input: return test_input
    if "(" in action_description:
  ...
  prompt_input = create_prompt_input(action_description, test_input)
  prompt = generate_prompt(prompt_input, prompt_template)
...
def run_gpt_prompt_event_triple(action_description, persona, test_input=None, verbose=False):
  ...
  def __chat_func_validate(gpt_response, prompt=""):
    try:
      gpt_response = __chat_func_clean_up(gpt_response, prompt="")
      # Event triple cleanup returns list with predicate and object (2 elements)
      # Subject is added from persona.name later, so we validate for 2 here
  ...
  prompt_input = create_prompt_input(action_description, persona, test_input)
  prompt = generate_prompt(prompt_input, prompt_template)
  ...
  if persona:
    ...
    # For testing without persona, output is already a list from cleanup
    if isinstance(output, list) and len(output) >= 2:
      output = ("name", output[0], output[1])
    # If output is already a tuple (fail_safe), keep it as is
...
def run_gpt_prompt_act_obj_desc(act_game_object, act_desp, persona, verbose=False):
  ...
  INPUT:
    act_game_object: Game object name
    act_desp: Action description
    persona: Persona instance
  OUTPUT:
    String describing what the object is doing
  """
...
def run_gpt_prompt_act_obj_event_triple(act_game_object, act_obj_desc, persona, verbose=False):
  ...
  INPUT:
    act_game_object: Game object name
    act_obj_desc: Object description
    persona: Persona instance
  OUTPUT:
    Tuple of (object, predicate, state)
  """
  ...
  example_output = "(bed, is, being made)"

  special_instruction = "The value for the output must ONLY contain the triple."
...
def run_gpt_prompt_new_decomp_schedule(persona, main_act_dur, truncated_act_dur,
  ...
  INPUT:
    persona: Persona instance
    main_act_dur: Main activity durations
    truncated_act_dur: Truncated activity durations
    start_time_hour: Start time
    end_time_hour: End time
    inserted_act: Inserted activity
    inserted_act_dur: Duration of inserted activity
  OUTPUT:
    List of [activity, duration] pairs
  """
  def create_prompt_input(persona, main_act_dur, truncated_act_dur, start_time_hour,
                          end_time_hour, inserted_act, inserted_act_dur, test_input=None):
    ...
    prompt_input = [persona_name, start_hour_str, end_hour_str, original_plan, persona_name,
                    inserted_act, inserted_act_dur, persona_name, start_hour_str,
                    end_hour_str, end_hour_str, new_plan_init]
    return prompt_input
  ...
  prompt_input = create_prompt_input(persona, main_act_dur, truncated_act_dur, start_time_hour,
                                     end_time_hour, inserted_act, inserted_act_dur, test_input)
  prompt = generate_prompt(prompt_input, prompt_template)

  example_output = "00:00 ~ 00:30 -- sleep\n00:30 ~ 01:00 -- prepare breakfast"

  special_instruction = "The output must be a list of revised schedule items in the format 'HH:MM ~ HH:MM -- activity description', one per line."
...
def run_gpt_prompt_decide_to_talk(persona, target_persona, retrieved, test_input=None,
  ...
  INPUT:
    ...
    target_persona: The target Persona to potentially talk to
  ...
  special_instruction = "Output must be 'yes' or 'no'. Answer in yes or no: <your answer>"
...
def run_gpt_prompt_decide_to_react(persona, target_persona, retrieved, test_input=None,
  ...
  example_output = "Option 1"

  special_instruction = "Output must be 'Option 1', 'Option 2', or 'Option 3'. Answer: <your answer>"
...
def run_gpt_prompt_create_conversation(persona, target_persona, curr_loc,
  ...
  example_output = 'Jane Doe: "Hi John!"\nJohn Doe: "Hello Jane!"'

  special_instruction = "Output should be a conversation with each line in the format: 'Name: \"utterance\"'"
...
def run_gpt_prompt_extract_keywords(persona, description, test_input=None, verbose=False):
  ...
  example_output = "friends, breakfast, park\nEmotive keywords: happy, excited"

  special_instruction = "Output should have two lines: first line for factual keywords (comma-separated), second line starting with 'Emotive keywords:' followed by emotive keywords (comma-separated)."
...
def run_gpt_prompt_keyword_to_thoughts(persona, keyword, concept_summary, test_input=None, verbose=False):
  ...
  INPUT:
    persona: The Persona instance
    keyword: Keyword to generate thoughts about
  ...
  example_output = "Jane is interested in learning more about the project"

  special_instruction = "Output should be a single thought statement."
...
def run_gpt_prompt_convo_to_thoughts(persona, init_persona_name, target_persona_name,
  ...
  INPUT:
    persona: The Persona instance
    init_persona_name: Name of initiator
    target_persona_name: Name of target
    convo_str: Conversation string
    fin_target: Target for thought generation
  OUTPUT:
    Thought string
  """
  ...
  example_output = "Jane found the conversation interesting"
...
def run_gpt_prompt_event_poignancy(persona, event_description, test_input=None, verbose=False):
  ...
  def create_prompt_input(persona, event_description, test_input=None):
    if test_input: return test_input
    prompt_input = [persona.scratch.name, persona.scratch.get_str_iss(),
                    persona.scratch.name, event_description]
    return prompt_input
...
def run_gpt_prompt_thought_poignancy(persona, event_description, test_input=None, verbose=False):
  ...
  INPUT:
    persona: The Persona instance
    event_description: Description of the thought
  OUTPUT:
    Integer from 1-10
  """
  def create_prompt_input(persona, event_description, test_input=None):
...
def run_gpt_prompt_chat_poignancy(persona, event_description, test_input=None, verbose=False):
  ...
  INPUT:
    persona: The Persona instance
    event_description: Description of the chat
  OUTPUT:
    Integer from 1-10
  """
  def create_prompt_input(persona, event_description, test_input=None):
...
def run_gpt_prompt_insight_and_guidance(persona, statements, n, test_input=None, verbose=False):
  ...
  OUTPUT:
    Dictionary mapping insights to evidence indices
  """
  def create_prompt_input(persona, statements, n, test_input=None):
    prompt_input = [statements, str(n)]
    return prompt_input
  ...
  example_output = "Jane is hungry (because of 1, 2, 3)"

  special_instruction = "Output should be numbered insights with evidence numbers in parentheses, e.g., '1. insight (because of 1, 2, 3)'"
...
def run_gpt_prompt_agent_chat(maze, persona, target_persona, curr_context,
  ...
  INPUT:
    ...
    target_persona: The target Persona
    curr_context: Current context
    init_summ_idea: Initiator's summary idea
    target_summ_idea: Target's summary idea
  OUTPUT:
    List of [speaker, utterance] pairs
  """
...
def run_gpt_prompt_generate_next_convo_line(persona, interlocutor_desc, prev_convo,
  ...
  INPUT:
    persona: The Persona instance
    interlocutor_desc: Description of the interlocutor
    prev_convo: Previous conversation
    retrieved_summary: Retrieved context summary
  OUTPUT:
    Next conversation line string
  """
  ...
  prompt_input = create_prompt_input(persona, interlocutor_desc, prev_convo, retrieved_summary, test_input)
  prompt = generate_prompt(prompt_input, prompt_template)
...
def run_gpt_prompt_generate_whisper_inner_thought(persona, whisper, test_input=None, verbose=False):
  ...
  example_output = "Jane is thinking about the project"

  special_instruction = "Output should be a single inner thought statement."
...
def run_gpt_prompt_planning_thought_on_convo(persona, all_utt, test_input=None, verbose=False):
  ...
  example_output = "Jane should prepare for the meeting"

  special_instruction = "Output should be a single planning thought statement."
...
def run_gpt_prompt_generate_safety_score(persona, comment, test_input=None, verbose=False):
  ...
  INPUT:
    persona: The Persona instance
    comment: Comment text to evaluate
  OUTPUT:
    Safety score or evaluation
  """
  ...
  prompt_input = create_prompt_input(comment, test_input)
  prompt = generate_prompt(prompt_input, prompt_template)

  example_output = "safe"

  special_instruction = "Output should be a safety evaluation."
...
def extract_first_json_dict(data_str):
  """
  Utility function: Extract the first JSON dictionary from a string.

  This is a helper function, not a GPT prompt function.
  """
...
def run_gpt_prompt_generate_iterative_chat_utt(maze, init_persona, target_persona,
  ...
  OUTPUT:
    Dictionary with "utterance" and "end" keys
  """
  ...
  prompt_input = create_prompt_input(maze, init_persona, target_persona, retrieved, curr_context, curr_chat, test_input)
  prompt = generate_prompt(prompt_input, prompt_template)

  example_output = '{"utterance": "Hello, how are you?", "end_conversation": "False"}'

  special_instruction = 'Output must be a JSON object with keys "utterance" (the next thing to say) and "end_conversation" (True/False).'
